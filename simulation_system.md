...
- 功能需求
  ...
  - 场景管理：台海作战场景的创建/编辑/软删除；支持地图选点录入我方无人机（坐标/高度/载荷空地导弹、空空导弹、机炮）与敌方侦察无人机、武装直升机、坦克、装甲车、军事基地等单位及编号。
...
2) 场景配置：在 `/create_scenario` 或 `/edit_scenario/<id>` 录入我方无人机（经纬高、载荷空地导弹/空空导弹/机炮）与敌方单位（类型、数量、坐标、编号），保存入库。
